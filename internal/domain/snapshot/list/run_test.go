...
// storedEngine holds one stored snapshot volume.
type storedEngine struct{ stubEngine }

func (storedEngine) Volumes(context.Context, docker.VolumeQuery) ([]docker.VolumeDetails, error) {
	return []docker.VolumeDetails{{
		Labels: docker.Labels{
			"pegged.snapshot.name":        "golden",
			"pegged.snapshot.source-port": "5432",
...
// failingEngine fails the volume listing SnapshotList issues.
type failingEngine struct{ stubEngine }

func (failingEngine) Volumes(context.Context, docker.VolumeQuery) ([]docker.VolumeDetails, error) {
	return nil, errSeam
}
...
func TestRun(t *testing.T) {
	tests := []struct {
		...
		name          string
		wantSnapshots []pgdocker.Snapshot
	}{
		{
			name:   "reports every stored snapshot",
			engine: storedEngine{},
			wantSnapshots: []pgdocker.Snapshot{{
				Labels: docker.Labels{
					"pegged.snapshot.name":        "golden",
				...
				Name:       "golden",
				Volume:     "pegged-snapshot-golden",
				Major:      "17",
				SourcePort: 5432,
			}},
		},
		{
			name:          "no snapshots stored",
			engine:        stubEngine{},
			wantSnapshots: []pgdocker.Snapshot{},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			must.NoError(err)
			want.Equal(tt.wantSnapshots, result.Snapshots)
		})
	}
}
