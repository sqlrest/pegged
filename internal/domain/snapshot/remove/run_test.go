...
// storedEngine holds one stored snapshot volume and records the removal.
type storedEngine struct {
	stubEngine
	removed *docker.VolumeName
}

func (storedEngine) Volumes(context.Context, docker.VolumeQuery) ([]docker.VolumeDetails, error) {
	return []docker.VolumeDetails{{
		Labels: docker.Labels{"pegged.snapshot.name": "golden"},
		Name:   "pegged-snapshot-golden",
	}}, nil
}

func (e storedEngine) RemoveVolume(
	_ context.Context,
	name docker.VolumeName,
	_ docker.VolumeRemoval,
) error {
	*e.removed = name
	return nil
}
...
func TestRun(t *testing.T) {
	removed := new(docker.VolumeName)
	tests := []struct {
		wantErr error
		engine  pgdocker.Engine
		name    string
		want    pgdocker.SnapshotName
		args    []domain.Argument
	}{
		{
			name:   "deletes the named snapshot",
			engine: storedEngine{removed: removed},
			args:   []domain.Argument{"golden"},
			want:   "golden",
		},
		{
		...
		{
			name:    "an unknown snapshot is not found",
			engine:  stubEngine{},
			args:    []domain.Argument{"missing"},
			wantErr: pgdocker.ErrSnapshotNotFound,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := Run(context.Background(), discardLogger(), Config{}, tt.args...)
			...
			must.NoError(err)
			want.Equal(tt.want, result.Deleted)
		})
	}

	assert.New(t).Equal(docker.VolumeName("pegged-snapshot-golden"), *removed,
		"the snapshot's volume is what gets removed")
}
