...
func TestCommandBinding(t *testing.T) {
	...
	err := testApp().Run(context.Background(), []string{
		"pegged", "start",
		"--port", "5433", "--image", "postgres:18-alpine", "--database", "app",
		"--user", "dev", "--password", "hunter2", "--snapshot", "golden",
		"--volume", "fresh", "--retain", "keep", "--listen", "0.0.0.0",
		"--init-sql", "/tmp/init", "--platform", "linux/arm64",
		"5433",
	})
	must.NoError(err)
...
func TestCommandEnvSources(t *testing.T) {
	...
	var gotArgs []string
	stubRun(t, &got, &gotArgs)
	t.Setenv("PEGGED_IMAGE", "postgres:16-alpine")
	t.Setenv("PGPORT", "6001")

	err := testApp().Run(context.Background(), []string{"pegged", "start"})
	must.NoError(err)

	want.Equal(domain.Config{Image: "postgres:16-alpine", Port: 6001}, got)
	want.Empty(gotArgs)
}
